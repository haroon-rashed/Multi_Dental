...
export const createGuestOrderWithUserAsync = createAsyncThunk(
  "order/createGuestOrderWithUserAsync",
  async (orderData) => {
    const result = await createGuestOrderWithUser(orderData);
    return result;
  }
);
...
const orderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createGuestOrderWithUserAsync.pending, (state) => {
        state.status = "pending";
      })
      .addCase(createGuestOrderWithUserAsync.fulfilled, (state, action) => {
        state.status = "fulfilled";
        state.currentOrder = action.payload.order;
        // You might also want to store user info if needed
      })
      .addCase(createGuestOrderWithUserAsync.rejected, (state, action) => {
        state.status = "rejected";
        state.errors = action.error;
      });
  },
