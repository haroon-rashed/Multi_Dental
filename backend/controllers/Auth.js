...
exports.signup = async (req, res) => {
  try {
    ...
    const newOtp = new Otp({
      ...
      otp: hashedOtp,
      expiresAt:
        Date.now() + parseInt(process.env.OTP_EXPIRATION_TIME || 300000), // 5 minutes default
    });
    await newOtp.save();
    ...
    await sendMail(
      ...
      `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <p style="color: #666; font-size: 12px;">Do not share this OTP with anyone for security reasons.</p>
            </div>
            `
    );
    ...
    // generating jwt token (but don't log them in until verified)
...
exports.verifyOtp = async (req, res) => {
  try {
    ...
    // checks if the otp is expired, if yes then deletes the otp and returns response accordingly
    ...
    res
      .status(500)
      .json({ message: "Some Error occurred during OTP verification" });
  }
};
...
exports.resendOtp = async (req, res) => {
  try {
    ...
    const newOtp = new Otp({
      ...
      otp: hashedOtp,
      expiresAt:
        Date.now() + parseInt(process.env.OTP_EXPIRATION_TIME || 300000),
    });
    await newOtp.save();
