...
const AddCategory = ({ editingCategory = null, onFinishEdit = () => {} }) => {
  ...
  useEffect(() => {
    if (editingCategory) {
      ...
      setImagePreview(editingCategory.categoryImage);
      setImage(null); // Avoid treating existing image as file input
    }
  }, [editingCategory]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!categoryName || (!image && !editingCategory)) return;
    ...
    try {
      if (editingCategory) {
        ...
        onFinishEdit(); // Callback to switch back to add mode
      } else {
  ...
  return (
    <AddCategoryContainer>
      <Title>{editingCategory ? 'Update Category' : 'Add New Category'}</Title>
