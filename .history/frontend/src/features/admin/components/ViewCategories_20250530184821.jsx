...
const ViewCategories = () => {
  ...
  // Debug: Log categories to see the data structure
  ...
  const getSubcategories = (parentId) => {
    const subcats = categories.filter(cat => {
      // Handle both string and number IDs, and both _id and id fields
      const catParentId = cat.parent_id;
      const normalizedParentId = String(parentId);
      const normalizedCatParentId = String(catParentId);
      
      return catParentId && normalizedCatParentId === normalizedParentId;
    });
    console.log(`Subcategories for parent ${parentId}:`, subcats);
    return subcats;
  };

  // Get only parent categories - Fixed logic
  const parentCategories = categories.filter(cat => {
    const hasNoParent = !cat.parent_id || cat.parent_id === null || cat.parent_id === '';
    console.log(`Category ${cat.name} - parent_id: ${cat.parent_id}, isParent: ${hasNoParent}`);
    return hasNoParent;
  });
  ...
  const renderSubcategories = (parentId) => {
    ...
    console.log(`Rendering subcategories for parent ${parentId}:`, subcategories);
      ...
      >
        ...
        <TableCell>
          <Typography variant="caption" color="text.secondary">
            Parent: {parentCategories.find(p => {
  ...
  return (
    <Box sx={{ mt: 4 }}>
      ...
      <Box sx={{ mb: 2, p: 2, backgroundColor: '#f5f5f5', borderRadius: 1 }}>
        ...
        <Typography variant="body2">Total Categories: {categories.length}</Typography>
        <Typography variant="body2">Parent Categories: {parentCategories.length}</Typography>
        <Typography variant="body2">Subcategories: {categories.filter(cat => cat.parent_id).length}</Typography>
      </Box>
