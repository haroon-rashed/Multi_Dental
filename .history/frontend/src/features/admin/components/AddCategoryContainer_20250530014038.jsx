...
const AddCategory = () => {
  ...
  const [parentId, setParentId] = useState('');  // <-- for parent category selection
  ...
  useEffect(() => {
    if (id) {
      ...
      if (existing) {
        ...
        setImage(null);
        setParentId(existing.parentId || ''); // <-- set parentId when editing
      }
    }
  }, [id, categories]);
  ...
  const handleSubmit = async (e) => {
    ...
    const formData = {
      ...
      parentId: parentId || null, // <-- assign null if no parent selected
    };

    try {
      if (id) {
        await updateCategory({ id, updatedData: formData });
      ...
      <form onSubmit={handleSubmit}>
        ...
        <FormGroup>
          ...
          >
            ...
            {categories
              .filter(cat => !cat.parentId)  // only list parent categories as possible parents
              .map(cat => (
